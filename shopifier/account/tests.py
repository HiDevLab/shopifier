...
class ModelAccountTests(TestCase):
    ...
    def test_create_super_user(self):
        ...
        with self.assertRaisesRegexp(ValueError, 'The given email must be set'): 
            user = User.objects.create_superuser("", self.ADMIN_PASS) 
...
class AccountTests(TestCase, BaseAccountTest):
    ...
    def test_base_account_methods(self):
        ...
        # anonymous invaiting
        self.post(self.invaite_url, data=fill, status_code=403)
        ...
        self.assertEqual(user.is_admin, False)
        self.assertEqual(user.is_staff, False)
        self.assertEqual(user.get_full_name(), '%s %s' % (self.FIRST_NAME, self.LAST_NAME))
        ...
        response = self.get(self.confirm_url, data=fill, status_code=200)
        ...
        self.get(self.confirm_url, data=fill, status_code=400)
        ...
        self.post(self.activate_url, data=fill, status_code=400)
        ...
        fill['token'] = sign
        response = self.post(self.activate_url, data=fill, status_code=200)
        ...
        response = self.post(self.password_change_url, data=fill, status_code=400)
        self.assertEqual(response.json['status'], _("Current password is wrong"))
        ...
        self.post(self.password_change_url, data=fill, status_code=200)
        ...
        self.post(self.login_url, data=fill, status_code=200)
        
        #logout
        self.get(self.logout_url, status_code=200)        
